...
const UserPage = ({match: {params: {_id}}, props = {}, posts = [], aboutMe, onLoadUserInf, onLoadUserPosts, postLike, postUnlike, onFollow, onUnfollow }) => {
    ...
    useEffect(()=>{
        ...
        if (Array.isArray(posts))ChangePostList([...myPosts,...posts])
        // <PostWrapper>
        if(!!aboutMe)ChangeView(myPosts.map(post => <Post key={post._id} userId={aboutMe._id} postId={post._id} title = {`${post.title}`} text={`${post.text}`} createdAt={`${post.createdAt}`} comments={post.comments} owner={post.owner} images={post.images} likes={post.likes} postLike={postLike} postUnlike={postUnlike} className="post"/> ))
        console.log('SmthToView, myPosts, posts',SmthToView, myPosts, posts)
    },[posts])
